...
* Author: kksou
* modded ga removed poi line 294
* removed backlink
*/
...
class Plugin_googleMaps_base {
	...
	function init_google_maps($row, $pluginParams, $is_mod) {
		...
		$param_list = array('api_key', 'width', 'height', 'zoom', 'template_uses_jquery, 'poi'');
		foreach($param_list as $var) {
			$this->$var = $pluginParams->$var;
		}

		$this->http = "http";
		if ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']=='on') || $_SERVER['SERVER_PORT'] == 443) $this->http = "https";
...
function display_{$this->mod}_gmap(id, centerLatitude, centerLongitude, startZoom, description, kml, control, maptype, show_marker, addoverview, addscale, addgoogle, streetview, styles) {
	var house = 'https://lrio.com/kml/icons/water.png';
...
//	var image = 'https://developers.google.com/maps/documentation/javascript/examples/full/images/beachflag.png';
	var house = 'https://lrio.com/kml/icons/surfer.png';
...
";
	...
	function gmap_code() {
		$js = "\n<script type=\"text/javascript\">
<!--
function init_{$this->mod}_gmap(id, addr, centerLatitude, centerLongitude, startZoom, description, kml, control, maptype, show_marker, addoverview, addscale, addgoogle, streetview, styles) {
    var map;
	if (addr!='') {
		...
		if (latlng_RegExp.test(addr2)) {
    		...
    		map = display_{$this->mod}_gmap(id, str[1], str[2], startZoom, description, kml, control, maptype, show_marker, addoverview, addscale, addgoogle, streetview, styles);
		} else {
			var geocoder = new google.maps.Geocoder();
			geocoder.geocode( { 'address': addr2}, function(results, status) {
		      if (status == google.maps.GeocoderStatus.OK) {
		        if (description=='') description = addr;
		      	map = display_{$this->mod}_gmap(id, results[0].geometry.location.lat(), results[0].geometry.location.lng(), startZoom, description, kml, control, maptype, show_marker, addoverview, addscale, addgoogle, streetview, styles);
		      } else {
    	...
    	map = display_{$this->mod}_gmap(id, centerLatitude, centerLongitude, startZoom, description, kml, control, maptype, show_marker, addoverview, addscale, addgoogle, streetview, styles);
    }
...
class Plugin_googleMaps extends Plugin_googleMaps_base {
	...
	function Plugin_googleMaps( &$row, $pluginParams, $is_mod=0 ) {
		 self::__construct( $row, $pluginParams, $is_mod );
	}
	...
	function output_map(&$row, $matches, $map_id) {
		...
		$output .= "<div style=\"width: {$width2};\"><p align=\"right\" style=\"padding:0 0 0 0;display:none;margin:0 0 0 0\"><a href=\"https://www.kksou.com/php-gtk2/Joomla-Gadgets/googleMaps-plugin.php\" style=\"color:#aaa;text-decoration: none;font-family:Tahoma, Arial, Helvetica, sans-serif;font-size:7pt;font-weight: normal;\">Powered by JoomlaGadgets</a></p></div>\n";
		if ($this->w3c) $output .= '<p>';
...
-->\n\n";
		...
		$output .= "\n"."<script type=\"text/javascript\" src=\"{$this->http}://maps.googleapis.com/maps/api/js?key={$this->api_key}&amp;language=".$this->lang.'"></script>';
